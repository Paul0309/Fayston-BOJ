        ...
        <div className="container mx-auto py-10 px-4">
            ...
            {problem ? (
                <div className="mb-3 text-sm text-neutral-300">
                    ...
                    <Link href="/status" className="ml-2 text-blue-400 hover:underline">
                        전체 보기
                    </Link>
                </div>
            ) : null}
            ...
            <form className="mb-4 grid grid-cols-1 md:grid-cols-6 gap-2">
                ...
                <select name="language" defaultValue={language} className="rounded border border-neutral-700 bg-neutral-900 text-neutral-100 px-3 py-2">
                    ...
                    {languageRows.map((row) => <option key={row.language} value={row.language}>{row.language}</option>)}
                </select>
                        ...
                        {submissions.map((sub) => {
                            ...
                            return (
                                <tr key={sub.id} className="hover:bg-neutral-800/70 transition-colors">
                                    <td className="px-4 py-3 font-mono text-neutral-300 text-xs"><Link href={`/status/${sub.id}`} className="hover:underline hover:text-blue-400">{sub.id.substring(0, 8)}</Link></td>
                                    <td className="px-4 py-3 font-medium text-blue-400"><Link href={`/user/${sub.user.id}`} className="hover:underline">{sub.user?.name || "Unknown"}</Link></td>
                                    <td className="px-4 py-3"><Link href={`/problem/${sub.problemId}`} className="font-medium hover:text-blue-400 hover:underline">{sub.problem.title}</Link></td>
                                    <td className="px-4 py-3">
                                        <span className={cn(
                                            "inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium",
                                            sub.status === "ACCEPTED" ? "bg-green-900/40 text-green-300" :
                                                sub.status === "PARTIAL" ? "bg-amber-900/40 text-amber-300" :
                                                    sub.status === "WRONG_ANSWER" ? "bg-red-900/40 text-red-300" :
                                                        sub.status === "PENDING" ? "bg-neutral-800 text-neutral-200" :
                                            ...
                                            {(sub.status === "TLE" || sub.status === "MLE" || sub.status === "RUNTIME_ERROR" || sub.status === "COMPILATION_ERROR") && <AlertTriangle className="w-3 h-3" />}
                                            {sub.status}{pendingProgress !== null ? ` ${pendingProgress}%` : ""}
                                        </span>
                                    </td>
                                    <td className="px-4 py-3 text-xs font-medium">{getVisibilityLabel(sub.codeVisibility)}</td>
                                    <td className="px-4 py-3 text-neutral-300 font-mono text-xs">{typeof sub.totalScore === "number" && typeof sub.maxScore === "number" ? `${sub.totalScore} / ${sub.maxScore}` : "-"}</td>
                                    ...
                                    <td className="px-4 py-3"><span className="font-mono text-xs uppercase bg-neutral-800 px-2 py-1 rounded inline-block">{sub.language}</span></td>
                                    <td className="px-4 py-3 text-xs">
                                        {canView ? <Link href={`/status/${sub.id}`} className="text-blue-400 hover:underline">코드 보기</Link> : <span className="text-neutral-500">비공개</span>}
                                    </td>
            ...
            <div className="mt-4 flex items-center justify-between text-sm text-neutral-300">
                ...
                <div className="flex gap-2">
                    <PageLink page={Math.max(1, currentPage - 1)} current={currentPage} params={searchParams}>이전</PageLink>
                    <PageLink page={Math.min(totalPages, currentPage + 1)} current={currentPage} params={searchParams}>다음</PageLink>
                </div>
            </div>
        </div>
    );
...
function Badge({ label, value, color }: { label: string; value: number; color: "neutral" | "blue" | "green" | "red" }) {
    const colorClass =
        color === "blue" ? "bg-blue-900/40 text-blue-300 border-blue-500/40" :
            color === "green" ? "bg-green-900/40 text-green-300 border-green-500/40" :
                color === "red" ? "bg-red-900/40 text-red-300 border-red-500/40" :
