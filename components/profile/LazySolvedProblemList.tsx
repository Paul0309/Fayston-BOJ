...
export default function LazySolvedProblemList({ title, items }: { title: string; items: SolvedProblemItem[] }) {
  ...
  return (
    <>
      ...
      {items.length === 0 ? <div className="px-6 py-6 text-sm text-neutral-300">{"\uC544\uC9C1 \uD574\uACB0\uD55C \uBB38\uC81C\uAC00 \uC5C6\uC2B5\uB2C8\uB2E4."}</div> : null}
          ...
          <div className="space-y-1 font-mono text-base leading-7 text-blue-300">
            {visibleGridRows.map((row, rowIndex) => (
              <div key={`row-${rowIndex}`} className="flex flex-wrap gap-x-2">
                {row.map((item) => (
                  <Link key={item.problemId} href={`/problem/${item.problemNumber}`} className="rounded px-1.5 py-0.5 transition hover:bg-blue-500/15 hover:text-blue-200" title={new Date(item.solvedAt).toLocaleString()}>
                    {item.problemNumber}
                  </Link>
                ))}
              </div>
        ...
        <>
          <ul className="divide-y divide-neutral-700">
            {pagedItems.map((item) => {
              ...
              return (
                <li key={item.problemId} className="px-6 py-4 transition hover:bg-neutral-800/40">
