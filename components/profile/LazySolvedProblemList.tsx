          ...
          <div className="space-y-1 font-mono text-base leading-7 text-blue-300">
            {visibleGridRows.map((row, rowIndex) => (
              <div key={`row-${rowIndex}`} className="flex flex-wrap gap-x-2">
                {row.map((item) => (
                  <Link key={item.problemId} href={`/problem/${item.problemId}`} className="rounded px-1.5 py-0.5 transition hover:bg-blue-500/15 hover:text-blue-200" title={new Date(item.solvedAt).toLocaleString()}>
                    {item.problemNumber}
                  </Link>
                ))}
              </div>
