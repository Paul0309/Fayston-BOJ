...
import { Menu, X, LayoutDashboard, Plus, Settings, Zap, Trophy } from "lucide-react";

const menuItems = [
  { href: "/admin", label: "대시보드", icon: LayoutDashboard },
  { href: "/admin#problem-create", label: "문제 생성", icon: Plus },
  { href: "/admin#contest-management", label: "대회 관리", icon: Trophy },
  { href: "/admin/ai-management", label: "AI 관리", icon: Zap },
  { href: "/admin#monitoring", label: "채점 모니터링", icon: Settings }
];
...
export default function AdminSidebar() {
  ...
  return (
    <>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-2.5 rounded-md transition text-sm font-medium ${
                  active ? "bg-blue-500 text-white" : "text-neutral-300 hover:bg-neutral-800"
                }`}
              >
                <Icon size={18} />
      ...
      {isOpen ? <div className="fixed inset-0 bg-black bg-opacity-50 z-30 lg:hidden" onClick={() => setIsOpen(false)} /> : null}
    </>
  );
}
